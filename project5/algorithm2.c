...
void pickUp(int me)
{
    ...
    wait(Fork[Left(me)]);
    if (read(Fork[Right(me)][0], NULL, 0) == 0)
    {                          // Check if right fork is available
        wait(Fork[Right(me)]); // If available, pick up the right fork
        printf("Philosopher %d has picked up both forks.\n", me);
    }
    else
    {
        signal(Fork[Left(me)]); // If not available, put down the left fork
        printf("Philosopher %d couldn't pick up both forks and is thinking.\n", me);
    }
}
